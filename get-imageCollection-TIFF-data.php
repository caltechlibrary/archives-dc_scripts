...
if (!isset($argv[1])) {
  exit("\n😵 error: supply an absolute path to a directory\n➡️  example: php get-imageCollection-TIFF-data.php /path/to/directory\n\n");
}

unlink($argv[1] . '/imageCollection-TIFF-data.csv');
file_put_contents($argv[1] . '/imageCollection-TIFF-data.csv', "pid,datastream,extension,format,mimetype,geometry,resolution,units,type,bitdepth,compression,filesize\n");
...
foreach ($iterator as $fileInfo) {
  ...
  // only act on TIFFs
  if (strpos($fileInfo->getFilename(), '_TIFF') !== FALSE) {

    $filepath = $fileInfo->getPath();
    $filename = $fileInfo->getFilename();
    $fileparts = explode('_', basename($filename, $fileInfo->getExtension()));

    $csv = fopen($argv[1] . '/imageCollection-TIFF-data.csv', 'a');
